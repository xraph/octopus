...
impl AdminHandler {
    ...
    pub fn handle(&self, method: &Method, path: &str) -> Result<Response<Full<Bytes>>> {
        ...
        match (method, path) {
            ...
            // Octopus-UI powered pages (modern UI)
            (&Method::GET, "/admin/octopus-ui") => self.octopus_ui_dashboard_page(),
            (&Method::GET, "/admin/octopus-ui/routes") => self.octopus_ui_routes_page(),
            (&Method::GET, "/admin/octopus-ui/health") => self.octopus_ui_health_page(),
            (&Method::GET, "/admin/octopus-ui/plugins") => self.octopus_ui_plugins_page(),
    ...
    // Octopus-UI Pages (Modern UI)

    fn octopus_ui_dashboard_page(&self) -> Result<Response<Full<Bytes>>> {
        use octopus_admin::octopus_ui_handlers::OctopusUiDashboardTemplate;
        use octopus_admin::ui_components;
        
        // Collect metrics
        let (total_requests, active_routes, avg_latency_ms, health_status) =
            if let Some(metrics) = &self.metrics_collector {
                let error_rate = metrics.global_error_rate();
                let health = if error_rate < 1.0 {
                    "healthy"
                } else if error_rate < 5.0 {
                    "warning"
                } else {
        ...
        let stats_cards = format!(
            ...
            ui_components::stats_card(
                ...
                "Configured routes",
                Some(r#"<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>"#)
            ),
            ui_components::stats_card(
                "Avg Latency",
            ...
            ui_components::stats_card(
                ...
                health_status,
                "System health",
                Some(r#"<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>"#)
            ),
        );

        // Get routes for table
        let router_routes = self.router.get_all_routes();
        let routes: Vec<octopus_admin::RouteInfo> = router_routes
            .iter()
            .take(5) // Show only first 5 routes on dashboard
            .enumerate()
            .map(|(_, route)| {
                let path = &route.path;
                let request_count = if let Some(metrics) = &self.metrics_collector {
                    metrics.route_stats(path)
                        .map(|s| s.request_count.load(Ordering::Relaxed))
                        .unwrap_or(0)
                } else {
                    0
                };
                let error_count = if let Some(metrics) = &self.metrics_collector {
                    metrics.route_stats(path)
                ...
                octopus_admin::RouteInfo {
                    id: format!("route-{}", route.path.replace('/', "-")),
                    path: route.path.clone(),
                    method: route.method.to_string(),
                    upstream: route.upstream_name.clone(),
                    request_count,
                    is_healthy: true,
                    avg_latency_ms: 0.0, // Latency tracking not available in this context
                    error_count,
                    last_accessed: None,
                }
            })
        ...
        let template = OctopusUiDashboardTemplate {
            stats_cards,
            routes_table,
            health_checks: "<p class=\"text-muted-foreground\">Health checks not yet implemented</p>".to_string(),
            plugins_grid: "<p class=\"text-muted-foreground\">Plugins not yet implemented</p>".to_string(),
        };

        self.render_template(template)
    }

    fn octopus_ui_routes_page(&self) -> Result<Response<Full<Bytes>>> {
        use octopus_admin::octopus_ui_handlers::OctopusUiRoutesTemplate;
        use octopus_admin::ui_components;
    ...
    fn octopus_ui_health_page(&self) -> Result<Response<Full<Bytes>>> {
        use octopus_admin::octopus_ui_handlers::OctopusUiHealthTemplate;
        use octopus_admin::ui_components;
        ...
        let template = OctopusUiHealthTemplate {
            ...
            overall_status: ui_components::health_status_badge("healthy"),
            uptime_24h: 99.9,
            uptime_7d: 99.8,
            uptime_30d: 99.7,
        };
    ...
    fn octopus_ui_plugins_page(&self) -> Result<Response<Full<Bytes>>> {
        use octopus_admin::octopus_ui_handlers::OctopusUiPluginsTemplate;
        use octopus_admin::ui_components;
