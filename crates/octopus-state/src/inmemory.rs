...
impl StateBackend for InMemoryBackend {
    ...
    async fn keys(&self, pattern: &str) -> Result<Vec<String>> {
        ...
        let keys: Vec<String> = self
            ...
            .iter()
            .filter(|entry| {
                !entry.value().is_expired() && re.is_match(entry.key())
            })
...
mod tests {
    ...
    async fn test_expire() {
        ...
        let success = backend.expire("key1", Duration::from_millis(50)).await.unwrap();
        assert!(success);
    ...
    async fn test_keys_pattern() {
        ...
        backend.set("user:1", b"alice".to_vec(), None).await.unwrap();
        backend.set("user:2", b"bob".to_vec(), None).await.unwrap();
        backend.set("session:1", b"data".to_vec(), None).await.unwrap();
