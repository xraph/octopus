...
async fn test_trace_context_generation() {
    ...
    assert_eq!(trace_ctx.trace_id.len(), 32, "Trace ID should be 32 hex chars");
    assert!(!trace_ctx.parent_span_id.is_empty(), "Should have span ID");
    assert_eq!(trace_ctx.parent_span_id.len(), 16, "Span ID should be 16 hex chars");
}
...
async fn test_trace_context_to_headers() {
    ...
    assert!(traceparent.to_str().unwrap().starts_with("00-"), "Should start with version 00");
}
...
async fn test_trace_context_without_header() {
    ...
    assert!(trace_ctx.is_none(), "Should return None without traceparent header");
}
...
async fn test_trace_context_invalid_header() {
    ...
    assert!(trace_ctx.is_none(), "Should return None for invalid traceparent");
}
...
async fn test_audit_event_minimal() {
    ...
    assert!(!event.event_id.is_empty(), "Should have auto-generated event ID");
}
