...
use super::*;
use octopus_proxy::shutdown::{ShutdownHandle, ShutdownConfig, ShutdownResult};
...
async fn test_concurrent_shutdown_listeners() {
    ...
    for listener in listeners {
        ...
        assert!(result.is_ok(), "All listeners should receive shutdown signal");
...
async fn test_shutdown_with_active_tasks() {
    ...
    for task in tasks {
        ...
        assert!(msg.contains("shutdown"), "Task should take shutdown path: {}", msg);
    }
    
    // Shutdown should complete gracefully
    let shutdown_result = timeout(Duration::from_millis(500), shutdown_task).await;
    assert!(shutdown_result.is_ok());
    assert_eq!(shutdown_result.unwrap().unwrap(), ShutdownResult::GracefullyCompleted);
}
