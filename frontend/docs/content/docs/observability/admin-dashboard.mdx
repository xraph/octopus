...
<Callout type="warn">
  **`allowed_ips` is not enforced by the runtime today.** The field is accepted
  in configuration but the gateway does not currently filter `/admin*` requests
  by source IP. Do not rely on it as your only protection — use `auth_provider`,
  and restrict access at the network layer (NetworkPolicy, firewall, or by not
  exposing the gateway's admin paths publicly).
</Callout>
