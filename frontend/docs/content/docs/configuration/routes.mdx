...
<Callout type="warn">
  Per-route `cors` **is enforced**: the CORS middleware runs in the chain and applies the matched
  route's override, falling back to the global [`cors`](/docs/configuration/auth) policy. Per-route
  `rate_limit` is currently **parsed and stored but not enforced** at request time — see
  [Middleware](/docs/middleware) for what is wired today.
</Callout>
