...
<Callout type="info">
  ...
  `cors` applies the matched route's override when a top-level [`cors`](/docs/configuration/auth)
  block is set (which adds the CORS middleware to the chain). See [Middleware](/docs/middleware).
</Callout>
