...
time every `reload_interval_secs` and, on change, reloads the certificate and key from disk into the
reloader's store, keeping the previous material if a reload fails.

<Callout type="warn">
  For the **file-based** acceptor, the reloader detects and reads updated files, but the
  certificate actively served by the running rustls acceptor is built once at startup and is **not**
  swapped in place by the file reloader. True zero-downtime certificate swapping is provided by the
  Kubernetes operator path below (`SwappableTlsAcceptor`). If you rotate file-based certificates,
  plan for a restart to pick up the new material reliably.
</Callout>
