...
<Callout type="warn">
  Known-inert: `admin.allowed_ips` is **parsed but not enforced** — only `admin.auth_provider`
  gates the admin surface today (and static assets bypass even that). Don't rely on the IP
  allowlist; restrict admin access at the network layer as well. See
